...
class PatreonCache(PrimaryTimedFunction):
    ...
    def call_function(self):
        try:
            ...
            logger.warning(
                "Patreon cache could not be retrieved from redis. Leaving existing cache."
            )
    ...
    def get_patrons(
        self, min_entitlement=0, max_entitlement=1000, exact_entitlement=None
    ):
        matching_patrons = {}
    ...
    def get_sorted_patrons(self, **kwargs):
        ...
        return sorted(
            all_patrons.values(), key=lambda x: x["entitlement_amount"], reverse=True
        )
    ...
    def get_monthly_kudos(self, user_id):
        ...
        if eamount == 23:
            ...
            logger.warning(
                f"Found patron '{user_id}' with non-standard entitlement: {eamount}"
            )
            return 0
