...
class CorruptPrompt(wze.BadRequest):
    def __init__(self, username, ip, prompt, message=None):
        if message:
            self.specific = message
        else:
...
class InvalidAPIKey(wze.Unauthorized):
    def __init__(self, subject, keytype="API"):
        if keytype == "Shared":
...
class TimeoutIP(wze.Forbidden):
    def __init__(self, ipaddr, ttl):
        ...
        self.log = (
            f"Client attempted to generate from {ipaddr} while in {ttl} seconds timeout"
        )
...
class KudosUpfront(wze.Forbidden):
    def __init__(self, kudos_required, username, message):
        self.specific = message
